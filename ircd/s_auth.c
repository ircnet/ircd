			    ...
			    */
				...
				if (IsRegisteredUser(cptr))
				{
					...
					(void) exit_client(cptr, cptr, &me,
									   cptr->reason ? cptr->reason : "Denied access");
				}
			}
			start = end;
		}
...
void start_auth(aClient *cptr)
{
	...
	Debug((DEBUG_NOTICE, "auth(%x) from %s %x %x",
		   cptr, inet_ntop(AF_INET6, (char *) &us.sin6_addr, ipv6string, sizeof(ipv6string)), us.sin6_addr.s6_addr[14],
	...
	if (bind(cptr->authfd, (struct SOCKADDR *) &us, ulen) >= 0)
	{
		...
		Debug((DEBUG_NOTICE, "auth(%x) to %s",
			   cptr, inet_ntop(AF_INET6, (char *) &them.sin6_addr, ipv6string, sizeof(ipv6string))));
#else
		Debug((DEBUG_NOTICE, "auth(%x) to %s",
	...
	else
	{
		...
		Debug((DEBUG_ERROR, "auth(%x) bind failed on %s port %d - %d",
			   cptr, inet_ntop(AF_INET6, (char *) &us.sin6_addr, ipv6string, sizeof(ipv6string)),
...
void send_authports(aClient *cptr)
{
	...
	Debug((DEBUG_SEND, "sending [%s] to auth port %s.113",
		   authbuf, inet_ntop(AF_INET6, (char *) &them.sin6_addr, ipv6string, sizeof(ipv6string))));
#else
	Debug((DEBUG_SEND, "sending [%s] to auth port %s.113",

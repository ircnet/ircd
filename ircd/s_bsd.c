...
int inetport(aClient *cptr, char *ip, char *ipmask, int port, int dolisten)
{
	...
	** in add_listener after we return from inetport()? --B. */
	...
	if (port)
	{
		...
		else if (!inetpton(AF_INET6, ip, server.sin6_addr.s6_addr))
...
int check_client(aClient *cptr)
{
	...
	Debug((DEBUG_DNS, "ch_cl: check access for %s[%s]",
		   cptr->name, inet_ntop(AF_INET6, (char *) &cptr->ip, ipv6string, sizeof(ipv6string))));
#else
	Debug((DEBUG_DNS, "ch_cl: check access for %s[%s]",
...
void close_connection(aClient *cptr)
{
								 ...
								 cptr->sockhost, CFLAG)))
	{
		...
		aconf->hold += (aconf->hold - cptr->since > HANGONGOODLINK) ? HANGONRETRYDELAY : ConfConFreq(aconf);
	...
	while (max--)
	{
		...
		*/
		...
		if ((iconf.caccept == 0 ||
			...
			cptr->confs->value.aconf != NULL && IsConfDelayed(cptr->confs->value.aconf))
		{
	...
	*/
	...
	else
	{
		...
		*/
		...
		if (IsPerson(cptr) &&
			DBufLength(&cptr->recvQ) > CLIENT_FLOOD && !is_allowed(cptr, ACL_CANFLOOD))
		{
...
#define CHECK_PFD(thisfd)                         \
	...
	(void) exit_client(cptr, cptr, &me, length >= 0 ? "EOF From client" : strerror(get_sockerr(cptr)));
} /* for(i) */
return ret;
}
...
{
	struct utmp ut;
	while (read(fd, (char *) &ut, sizeof(struct utmp)) == sizeof(struct utmp))
	{
...
void summon(aClient *who, char *namebuf, char *linebuf, char *chname)
{
	 ...
	 */
	if ((linebuf[0] != 't' || linebuf[1] != 't' || linebuf[2] != 'y') && (linebuf[0] != 'c' || linebuf[1] != 'o' || linebuf[2] != 'n') && (linebuf[0] != 'p' || linebuf[1] != 't' || linebuf[2] != 's')
#ifdef HPUX
...
#ifdef INET6
							n, inetntop(AF_INET6, (char *) &from.sin6_addr, ipv6string, sizeof(ipv6string)),
#else

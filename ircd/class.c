...
int get_sendq(aClient *cptr, int bursting)
{
	...
	if (cptr->serv && cptr->serv->nline)
		sendq = bursting && MaxBSendq(cptr->serv->nline->class) ? MaxBSendq(cptr->serv->nline->class) : MaxSendq(cptr->serv->nline->class);
		...
		for (tmp = cptr->confs; tmp; tmp = tmp->next)
		{
			if (!tmp->value.aconf ||
				!(cl = tmp->value.aconf->class))
				continue;
			sendq = bursting && MaxBSendq(cl) ? MaxBSendq(cl) : MaxSendq(cl);
			break;
		}
	return sendq;
}

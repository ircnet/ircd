				...
				int parc, char *parv[])
{
	...
	*/
	...
	if (acptr)
	{
		...
		if (IsService(sptr) && (IsServer(acptr->from) && match(sptr->service->dist, acptr->name) != 0 && match(sptr->service->dist, acptr->serv->sid) != 0))
		{
...
int m_nick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (MyConnect(sptr) && IsUnknown(sptr) && nick[0] == '0' && nick[1] == '\0')
	{
	...
	if (IsServer(cptr) && isdigit(nick[0]) && !strncasecmp(me.serv->sid, nick, SIDLEN))
	{
	...
	*/
	if (!IsServer(cptr))
	{
		...
		*/
		sendto_one(sptr, replies[(delayed) ? ERR_UNAVAILRESOURCE : ERR_NICKNAMEINUSE],
					 ...
					 char *parv[], int notice)
{
		 ...
		 nick = strtoken(&p, NULL, ","), penalty++)
	{
		...
		** nickname addressed?
		*/
		if (((IsServer(cptr) || IsService(cptr)) && (acptr = find_uid(nick, NULL))) ||
...
static void who_find(aClient *sptr, char *mask, struct who_opts *opts)
{
	...
	for (acptr = client; acptr; acptr = acptr->next)
	{
		...
		if (IsInvisible(acptr) && (acptr != sptr) && !(MyConnect(acptr) && MyConnect(sptr) && IsAnOper(sptr)))
		{
			continue;
		}
	...
	*/
	if (IsService(acptr))
	{
		sendto_flag(SCH_KILL, "Received KILL message for %s[%s]. "
							  "From %s Path: %s!%s",
					acptr->name,
					isdigit(acptr->service->servp->sid[0]) ? acptr->service->servp->sid : "2.10", parv[0], inpath,
					path);
	}
	else
	{
		sendto_flag(SCH_KILL, "Received KILL message for "
					...
					isdigit(acptr->user->servp->sid[0]) ? acptr->user->servp->sid : "2.10", parv[0], inpath,
					path);
	}
	...
	if (IsOper(sptr))
	{
		if (IsService(acptr))
		{
			syslog(LOG_DEBUG, "KILL From %s For %s[%s] Path %s!%s",
				   parv[0], acptr->name,
				   isdigit(acptr->service->servp->sid[0]) ? acptr->service->servp->sid : "2.10",
				   inpath, path);
		}
		else
		{
			syslog(LOG_DEBUG, "KILL From %s For %s!%s@%s[%s/%s] "
				   ...
				   acptr->user->username, acptr->user->host,
				   acptr->user->servp->bcptr->name,
				   isdigit(acptr->user->servp->sid[0]) ? acptr->user->servp->sid : "2.10",
				   inpath, path);
		}
	}
#endif

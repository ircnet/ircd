...
static time_t try_connections(time_t currenttime)
{
	...
	else if (allheld == 0) /* disable AC only when some C: got checked */
	{
		...
		if (kflag == -1)
		{
			...
			if (!BadPtr(reason))
				sprintf(buf, "Kill line active: %.80s",
						reason);
			(void) exit_client(cptr, cptr, &me, (reason) ? buf : "Kill line active");
		}
	}
...
static time_t check_pings(time_t currenttime)
{
	...
	for (i = highest_fd; i >= 0; i--)
	{
		...
		*/
		if ((currenttime - lkill > TIMEDKLINES) && IsPerson(cptr) && !IsKlineExempt(cptr))
		{
...
static void open_debugfile(void)
{
	...
	if (debuglevel >= 0)
	{
		...
		Debug((DEBUG_FATAL, "Debug: File <%s> Level: %d at %s",
			   ((!(bootopt & BOOT_TTY)) ? IRCDDBG_PATH : (isatty(2) && ttyname(2)) ? ttyname(2)
																				   : "FD2-Pipe"),

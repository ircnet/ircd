		...
		switch (*curr)
		{
			 ...
			 */
				...
				if (whatt == MODE_ADD)
				{
					lp = &chops[opcnt++];
					lp->value.cptr = who;
					lp->flags = (*curr == 'O') ? MODE_UNIQOP : (*curr == 'o') ? MODE_CHANOP
																			  : MODE_VOICE;
					lp->flags |= MODE_ADD;
...
*/
			...
			case 'k':
				...
				** (prefer "highest" key) */
					if (IsServer(sptr) && IsBursting(sptr) &&
						*mode->key && strncmp(mode->key, *parv, (size_t) KEYLEN) >= 0)
						break;
					if (ischop)
					{
						if (**parv == ':')
			...
			default:
				*penalty += 1;
				...
				if (*ip)
				{
					if ((*ip == MODE_ANONYMOUS || *ip == MODE_REOP) && whatt == MODE_ADD && !IsServer(sptr))
						sendto_one(cptr,
		...
		for (; i < opcnt; i++)
		{
			...
			switch (lp->flags & MODE_WPARAS)
			{
				case MODE_CHANOP:
					...
					ucp = lp->value.cptr->user ? lp->value.cptr->user->uid : cp;
					break;
				case MODE_UNIQOP:
					c = 'O';
					cp = lp->value.cptr->name;
...
static int can_join(aClient *sptr, aChannel *chptr, char *key)
{
...
#ifdef CLIENTS_CHANNEL
	if (*chptr->chname == '&' && !strcmp(chptr->chname, "&CLIENTS") && is_allowed(sptr, ACL_CLIENTS))
		return 0;
#endif
	if (*chptr->chname == '&' && (!strcmp(chptr->chname, "&DEBUG") || !strcmp(chptr->chname, "&ERRORS") || !strcmp(chptr->chname, "&HASH") || !strcmp(chptr->chname, "&LOCAL") || !strcmp(chptr->chname, "&OPER")
#if defined(USE_IAUTH)
	...
	if ((chptr->mode.mode & MODE_INVITEONLY) && !match_modeid(CHFL_INVITE, sptr, chptr) && (lp == NULL))
		...
		*/
		...
		)
		{
			sendto_match_servs_v(chptr, cptr, SV_UID,
								 ...
								 flags & CHFL_UNIQOP ? "@@" : flags & CHFL_CHANOP ? "@"
																				  : "",
	...
	** remote clients that can join channels. --B. */
	if (parv[2][0] == '.' && parv[2][1] == '\0')
	{
		...
		** locked, can it? --B. */
		...
		{
			chptr->history = timeofday + (*chptr->chname == '!' ? LDELAYCHASETIMELIMIT : DELAYCHASETIMELIMIT);
			istat.is_hchan++;
...
int m_kick(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	maxlen = BUFSIZE - MAX(strlen(sender), strlen(sptr->name)) - strlen(comment) - 10; /* ":", " KICK ", " " and " :" */
		...
		for (tmp2 = tmp; (user = strtoken(&p2, tmp2, ",")); tmp2 = NULL)
		{
			...
			if (IsMember(who, chptr))
			{
				...
				** on the right side, before the loop. */
				if (strlen(nbuf) + (who->user ? UIDLEN : strlen(who->name)) >= clen)
				{
				...
				strcat(nbuf, who->user ? who->user->uid : who->name);
		...
		if (!topic) /* only asking  for topic  */
		{
			...
			else
			{
				...
				if (chptr->topic_t > 0)
					sendto_one(sptr, replies[RPL_TOPIC_WHO_TIME],
							   ME, BadTo(parv[0]), chptr->chname,
							   IsAnonymous(chptr) ? "anonymous!anonymous@anonymous." : chptr->topic_nuh, chptr->topic_t);
#endif
			}
		}

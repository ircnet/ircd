...
int parse(aClient *cptr, char *buffer, char *bufend)
{
	...
	*/
	...
	if (fhandler != m_nop && fhandler != m_nopriv && fhandler != m_unreg &&
		mptr->minparams > 0 &&
	...
	** a LOCAL user client.
	*/
	if ((ret > 0) && IsRegisteredUser(cptr) && (bootopt & BOOT_PROT) && !is_allowed(cptr, ACL_NOPENALTY))
	{
		cptr->since += ret;
		/* only to lurk
		sendto_one(cptr,
...
static void remove_unknown(aClient *cptr, char *sender)
{
	...
	if ((index(sender, '.') && !index(sender, '@')) || sid_valid(sender))
	{
